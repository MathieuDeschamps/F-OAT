...
.put( function() {
  ...
  if(!old_xml_string){
    ...
    this.response.end("ressource does not exist");
  }
 ...
 fs.writeFile(dir+"/annotation.xml",old_xml.html(), function(err) {
   if(err) {
     console.log("Faile to save ressource for project: "+this.params._id);
     this.response.statusCode = 500;
     this.response.end("Failed to save ressource");
   }
   else{
     console.log("File saved successfully! id: "+this.params._id);
     this.response.statusCode = 200;
