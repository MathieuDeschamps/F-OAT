...
.put( function() {
  ...
  var new_data = '\n\t\t<'+json.idExtractor+' name="'+extractor.name+'" version="'+version+'">\n\t\t</'+json.idExtractor+'>\n';
...
.put( function() {
  ...
  fs.access(dir+"/descriptor.xml", fs.F_OK, (err) => {
    if(err){
      console.log("descriptor version "+version+" does not exists, put descriptor.xml");

      fs.writeFile(dir+"/descriptor.xml",json.data, function(error,res){
       if(error) {
         console.log("Failed to save descriptor xml for extractor: "+idExtractor+" version "+version);
         response.statusCode = 500;
         response.end("Failed to save ressource");
       }
       else{
