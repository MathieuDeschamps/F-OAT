...
//On recoit ici le fichier JSON dans le body et on va le convertir en XML pour l'enregistrer
Router.route( "/receiveXML/:projectName", { where: "server" } )
...
.post( function(data) {
  ...
  fs.writeFile(dir+"/"+this.params.projectName+".xml", json2xml(data.body), function(err) {
    if(err) {
