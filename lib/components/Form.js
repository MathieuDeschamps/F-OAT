export class Form{
  ...
  static browseXML(XMLObject, iNode, parentNode){
    if(XMLObject == undefined){
      ...
      $(XMLObject).each(function(i,e0){
        ...
        node += '<div class="blue-text text-darken-2 row"><div class="col s1">' + nodeName + '</div>'
        node += '<i class="col s1 offset-s10 material-icons">keyboard_arrow_left</i></div></a>'
  ...
  static browseXMLBis(XMLObject, iNode, parentNode){
    if(XMLObject == undefined){
      alert("No xml to display.")
    }else{
      var node
      ...
      $(XMLObject).each(function(i,e0){
        ...
        node += '<li>'
        node += '<div class="collapsible-header white-text blue darken-4 row">'
        node += '<div class="col s1">' + nodeName + '</div>'
        node += '<i class="col s1 offset-s10 material-icons">keyboard_arrow_left</i>'
        node += '</div>'
      ...
      if($(XMLObject).children() != undefined){
        $(XMLObject).children().each(function(j,e){
          Form.browseXMLBis(e, iNode + "-"+ j, '#body' + iNode)
        })
      }
    }
  }
